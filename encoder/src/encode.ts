...
function generateRows(
  ...
  for (let h = 0; h < hours; h++) {
    ...
    for (let i = 0; i < rowsPerHour; i++) {
      ...
      const baseEntity = useOverlap ? rand(overlapPool) : `${agency}-${topic}-${zone}-${h}-${i}-${Math.random()}`;
...
async function writeParquet(outPath: string, rows: TapeRow[]) {
  const schema = new (parquet as any).ParquetSchema({
  ...
  const writer = await (parquet as any).ParquetWriter.openFile(schema, outPath);
